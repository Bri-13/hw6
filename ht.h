...
void HashTable<K,V,Prober,Hash,KEqual>::insert(const ItemType& p)
{
    ...
    // if no location available, throw error (shouldn't happen)
    HASH_INDEX_T location = probe(p.first);
    if(location == npos){
        throw std::logic_error("no available location to insert the item");
    }
